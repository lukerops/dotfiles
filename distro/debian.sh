...
sudo apt install --no-install-suggests $(cat ./distro/pkgs/*core.system.debian.pkgs | xargs)
...
sudo apt install --no-install-suggests $(cat ./distro/pkgs/pipewire.audio.debian.pkgs | xargs)
...
if [[ $(lspci | egrep -i 'wifi|wireless') ]]; then
    sudo apt install --no-install-suggests $(cat ./distro/pkgs/wireless.system.debian.pkgs | xargs)
fi

# instala o bluetooth se encontrar uma dispositivo bluetooth
if [[ $(lsusb | egrep -i 'bluetooth') ]]; then
    sudo apt install --no-install-suggests $(cat ./distro/pkgs/bluetooth.system.debian.pkgs | xargs)
fi

# instala o gnome se for o desktop escolhido
if [[ $DESKTOP == 'gnome' ]]; then
    sudo apt install --no-install-suggests $(cat ./distro/pkgs/gnome.desktop.debian.pkgs | xargs)
fi

# instala o sway se for o desktop escolhido
if [[ $DESKTOP == 'sway' ]]; then
    sudo apt install --no-install-suggests $(cat ./distro/pkgs/sway.desktop.debian.pkgs | xargs)
